...
public class HackForums extends Thread {
...
//        convertTimeToSecsArray();
        ...
        hackForumDriver.navigate().to("https://hackforums.net/search.php");
        WebElement searchBox = hackForumDriver.findElementByXPath("//input[contains(@class, 'textbox')]");
        searchBox.sendKeys("*ware");
        WebElement radioSearch = hackForumDriver.findElementByXPath("//input[contains(@name, 'postthread')]");
        radioSearch.click();

        WebElement byPostDate = hackForumDriver.findElementByXPath("//select[contains(@name, 'postdate')]");
        byPostDate.sendKeys("Find post from any post date");
...
//
//        hackForumDriver.navigate().to("https://hackforums.net/search.php");
//         searchBox = hackForumDriver.findElementByXPath("//input[contains(@class, 'textbox')]");
//        searchBox.sendKeys("ransomware");
//         radioSearch = hackForumDriver.findElementByXPath("//input[contains(@name, 'postthread')]");
//        radioSearch.click();
...
//        scrapeThreads(threadList,userList);
        ...
        forumList.add(hackForumDriver.getCurrentUrl());
        ...
        try{
            ...
            for (String forumlink: forumList) {
                ...
                List<WebElement> commentPagesInfo = hackForumDriver.findElementsByClassName("pages");
                String commentPagesData = (String) ((JavascriptExecutor) hackForumDriver).executeScript("return arguments[0].innerHTML;", commentPagesInfo.get(0));

                // Extract Thread number
                Pattern threadNumberPattern = Pattern.compile("Pages \\((.+?)\\):");
                Matcher matcher = threadNumberPattern.matcher(commentPagesData);
                matcher.find();
                totalThreads = Integer.valueOf(matcher.group(1));
                System.out.print(totalThreads);



                Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("HackForums-" + dateString + ".csv"), "utf-8"));
                ...
                do {
                    // List<WebElement> threadsList = hackForumDriver.findElements(By.xpath("//a[contains(@class, ' subject_ ')]"));

                    List<WebElement> threadsList = hackForumDriver.findElements(By.xpath("//td[contains(@class, 'trow1 forumdisplay_regular')]"));


                    for(WebElement threadHtml: threadsList){

                        String nseUrl = (String) ((JavascriptExecutor) hackForumDriver).executeScript("return arguments[0].innerHTML;", threadHtml);
                        ...
                        try {
                            ...
                            Pattern threadNamePattern = Pattern.compile("<a href=\"(.+?)\" class=\" subject_.+?\" id=\".+?\">(.+?)<\\/a>");
                            ...
                            thread.setThreadLink("https://hackforums.net/" + threadLink );




                            // Extract Thread Creator
                            Pattern threadCreatorPattern = Pattern.compile("<a href=\"https:\\/\\/hackforums.net\\/member\\.php\\?action=profile&amp;uid=(.+?)\">(.+?)<\\/a>");
                            matcher = threadCreatorPattern.matcher(Url);
                            matcher.find();
                            user = new User(matcher.group(2).trim());
                            user.setUserPageLink("https://hackforums.net/member.php?action=profile&uid=" + matcher.group(1).trim());
...
//                            userList.add(user);
                    ...
                    pageNumber++;
                    try {
                        hackForumDriver.findElementByClassName("pagination_next").click();
...
//                } while(pageNumber < 2  ); // Used to test Final Function
                } while(pageNumber < totalThreads + 1  );
                ...
                try {
                    ...
                    writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("HackForumThread-" + dateString + ".csv"), "utf-8"));
...
//        radioSearch.click();
        ...
        try {
            Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("ThreadsHackForumDetails-" + dateString + ".csv"), "utf-8"));
            ...
            for(Thread thread: threadList){
                ...
                try{
                    ...
                    hackForumDriver.navigate().to(thread.threadLink);


                    //Gathering data on comments for this particular thread

                    List<WebElement> commentTimeInfo = hackForumDriver.findElements(By.xpath("//td[contains(@class, 'tcat')]//div[contains(@class, 'float_left smalltext')]"));
                    List<WebElement> commentorNamesInfo = hackForumDriver.findElements(By.xpath("//td[contains(@class, 'post_author')]//strong//span[contains(@class, 'largetext')]"));
                    List<WebElement> commentBodyInfo = hackForumDriver.findElements(By.xpath("//div[contains(@class, 'post_body')]"));
                    ...
                    for ( int i=0; i < 11 ; i++){  // We expect to have 10 comments per page
                        ...
                        if( commentPagesNumber<0 ){
                            ...
                            if(commentorNamesInfo.size()==10){
                                ...
                                hackForumDriver.findElementByClassName("pagination_next").click();
                                commentTimeInfo = hackForumDriver.findElements(By.xpath("//td[contains(@class, 'tcat')]//div[contains(@class, 'float_left smalltext')]"));
                                commentorNamesInfo = hackForumDriver.findElements(By.xpath("//td[contains(@class, 'post_author')]//strong//span[contains(@class, 'largetext')]"));
                                commentBodyInfo = hackForumDriver.findElements(By.xpath("//div[contains(@class, 'post_body')]"));
                                commentPagesNumber = commentorNamesInfo.size();
                                commentPagesNumber--; // count down the comments so you know when to move to next page or next thread
                                commentNumber++; //count the number of comments parsed so you know that the first one is the creator
                                i = 0;
                        ...
                        commentTimeData = (String)((JavascriptExecutor)hackForumDriver).executeScript("return arguments[0].innerHTML;", commentTimeInfo.get(i));
                        commentorNamesData = (String)((JavascriptExecutor)hackForumDriver).executeScript("return arguments[0].innerHTML;", commentorNamesInfo.get(i));
                        commentBodyData = (String)((JavascriptExecutor)hackForumDriver).executeScript("return arguments[0].innerHTML;", commentBodyInfo.get(i));
